...
div_op:
    mov rax, rdi       ; Coloca o dividendo em rax
    xor rdx, rdx       ; Limpa rdx antes da divisão
    div rsi            ; Divide rax pelo divisor (num2)

    ; Aqui, rax contém o resultado da divisão
    mov [result], rax  ; Armazena o resultado

    ; Imprime o resultado
    mov rax, 1         ; syscall: write
    mov rdi, 1         ; file descriptor: stdout
    mov rsi, msg_format ; ponteiro para a mensagem
    mov rdx, 20        ; tamanho da mensagem (ajuste conforme necessário)
    syscall

    ; Imprime o valor do resultado
    mov rax, [result]  ; Move o resultado para rax
    call print_number   ; Chama a função para imprimir o número

    jmp exit           ; Sai do programa

	
div_error:
    ; Código para tratar o erro de divisão por zero
    ; (exibir mensagem ou definir um valor de erro)
    jmp exit            ; Sai do programa
