...
div_op:
    ...
    je div_error        ; Se for zero, ira para div_error
    xor rdx, rdx        ; Limpa rdx antes da divisão
    mov rax, rbx        ; Coloca o primeiro número (num1) em rax (dividendo)
    ; Não precisa mover rsi, rsi já contém o segundo número (num2)
    div rsi             ; Divide rax (num1) pelo segundo número (num2) em rsi
    jmp print_result     ; Continua para a impressão do resultado
...
str_to_int:
    ...
    mov rax, 0
    mov rcx, 10
	mov rdi, rbx  ; O primeiro número convertido
	call print_num_in_rdi ; Chamando uma função para imprimir o valor em rdi
	mov rdi, rax  ; O segundo número convertido
	call print_num_in_rdi ; Chamando uma função para imprimir o valor em rdi
.loop:
    movzx rdx, byte [rsi]
...
print_num_in_rdi:
    ; Imprime o número armazenado em rdi
    ; Supondo que o número já está em rdi em formato de inteiro
    ; Chame a função int_to_str para converter o número para string
    mov rax, rdi
    call int_to_str
    ; Agora, imprime a string armazenada no result
    mov rax, 1
    mov rdi, 1
    mov rsi, result
    mov rdx, 11 ; Tamanho do buffer (modifique conforme necessário)
    syscall
    ret
